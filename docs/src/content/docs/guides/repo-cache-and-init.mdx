...
```typescript
import { initBareRepo } from '@nandan-varma/git-edge';
...
Accepts any `fs` — `node:fs`, `@nandan-varma/git-fs-s3`-backed, or in-memory.
...
```typescript
import { estimateRepoSize } from '@nandan-varma/git-edge';
